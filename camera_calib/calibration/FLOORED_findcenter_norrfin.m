...
%            hold on; plot(yc,xc,'r.');
            ...
            %[RRfin,ss]=calibrate_norrfin(Xt, Yt, Xp_abs, Yp_abs, xc, yc, taylor_order, ima_proc, RRfin);
            if RRfin==0
                MSEA(ic,jc)=inf;
                continue;
            end
...
%    drawnow;
    ...
    xc=xreg(indMSE(ceil(length(indMSE)/2)));
    yc=yreg(indMSE(ceil(length(indMSE)/2)));
