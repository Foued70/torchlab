...
leasqrfunc = @(ss, x0) prova(x0, ss, int_par, Xp_abs(:,:,i), Yp_abs(:,:,i), M, width, height);
    targresults=zeros(1, length(Xt))';
    
    [f1, x0, cvg, iter, corp, covp, covr, stdresid, Z, r2] =leasqr(ss, targresults,x0, leasqrfunc, 0.0001, 10000, ones(size(targresults)), 0.001*ones(size(x0)), @dfdp);
...
f0=[1,1,ones(1,size(ss,1))];
lb=[0,0,zeros(1,size(ss,1))];
ub=[2,2,2*ones(1,size(ss,1))];
...
leasqrfunc = @(ss, f0) prova3([f0(1:2)', c, d, e, f0(3:end)'],xc, yc, ss, RRfin, ima_proc, Xp_abs, Yp_abs, M, width, height);
targresults=zeros(1, length(ima_proc)*length(Xt))';
          
[f1, ssout, cvg, iter, corp, covp, covr, stdresid, Z, r2] =leasqr(ss, targresults,f0', leasqrfunc, 0.0001, 10000, ones(size(targresults)), 0.001*ones(size(f0)), @dfdp, options);
