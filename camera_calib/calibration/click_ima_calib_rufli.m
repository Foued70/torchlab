...
callString = (['FindCorners.exe -w ' num2str(n_sq_x+1) ' -h ' num2str(n_sq_y+1) ' -m ' num2str((n_sq_x+1) * (n_sq_y+1)) ' pictures.txt']);
...
numCornerThreshold = (n_sq_x+1)*(n_sq_y+1);
numOfFoundCorners = 0;
cornerNumber = 0;
...
if (1!=1)
    ...
    while true
        ...
        xdel = xdel*ones( (n_sq_x+1)*(n_sq_y+1),1 );
        ydel = ydel*ones( (n_sq_x+1)*(n_sq_y+1),1 );
        ...
        if( nearestCornerID == (n_sq_x+1)*(n_sq_y+1) )
...
%    set(findobj('type', 'text'), 'color', 'red');
%    axis([min_x max_x min_y max_y]);
%    draw_axes(Xp_abs(:,:,kk), Yp_abs(:,:,kk),n_sq_y);
