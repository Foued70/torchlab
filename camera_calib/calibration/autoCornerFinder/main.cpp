...
int main( int argc, char** argv )
{
    ...
    printf("elem_size: %d, board_size_width: %d, board_size_height: %d, sizeofVcPoint2D32f: %d\n", elem_size, board_size.width, board_size.height, sizeof(CvPoint2D32f));
	image_points_seq = cvCreateSeq( 0, sizeof(CvSeq), elem_size, storage );

    /*const char* cornerXFileName = "cToMatlab/cornersX.txt";
    const char* cornerYFileName = "cToMatlab/cornersY.txt";
    FILE* cornerXFile = fopen( cornerXFileName, "w" );
    FILE* cornerYFile = fopen( cornerYFileName, "w" );
    char xbuffer[256];
    char ybuffer[256];*/
	...
	for(int j = 1;; j++)
	{
		...
		else
		{
            ...
            printf("found: %d, count:%d, image_points_buf: [%d x %d]\n", found, count, image_points_buf->x, image_points_buf->y);
    ...
    /*const char* cornerInfoFileName = "cToMatlab/cornerInfo.txt";
    FILE* cornerInfoFile = fopen( cornerInfoFileName, "w" );
    char buffer[256];
    snprintf(buffer, 16, "%d %d", board_size.width, board_size.height);
    fputs(buffer, cornerInfoFile);
    fclose(cornerInfoFile);
