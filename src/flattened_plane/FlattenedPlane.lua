...
function FlattenedPlane:__init(base_dir, sweep_name, tree, index)
    ...
    if not(util.fs.is_dir(path.join(base_dir, Sweep.SWEEP))) then
        error("expected SWEEP to exist in base_dir " .. path.join(base_dir, Sweep.SWEEP))
    end
...
function FlattenedPlane:addPlane(sweep, H)
    ...
    local plane_norm = pc:get_global_normal_map(H)
