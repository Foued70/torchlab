...
function FloorTransformation.findTransformationOurs(image1Path, image2Path)
  ...
  local trans2 = {}
  local combined = {}
  for i=1,best_pts:size()[1] do
      local trans1_i, trans2_i, combined_i = image.warpAndCombineWithBorders(best_transformations[i], img_src, img_dest)
      trans1[i] = trans1_i
      trans2[i] = trans2_i
  ...
  -- combinedImage = opencv.imgproc.combineImages(img_src, img_dest, transform, 2700, 2700,500, 1700 )
   ...
   return best_transformations, trans1, trans2, combined 
end
