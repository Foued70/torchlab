...
function ArcFile:needs_upload() 
  local local_mod_time = sys.fstat(self.filename)
...
function ArcFile:upload()
  local result = net.Depot.put_arc_file(self.path, self.filename)
  if result then 
    self.server_mod_time = result
    util.utime.set_mod_time(self.filename, self.server_mod_time)
  end
end
