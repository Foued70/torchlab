--[[
	Extract Planes using a method similar to:
	...
	TODO: Clean up, set up simple framework to construct various extraction pipelines
]]--
...
print("n_reference_points: ", params.n_reference_points)
print("n_iter: ", params.n_iter)

print("sample_bounds: ", params.sample_bounds)

-- Extract planes for all scans and save output 
scan_start = 6
scan_end = 27
for scan_num = scan_start, scan_end do
	print("Extracting Planes for scan_num: ", scan_num)

	-- Filenames and whatnot
	scan_fname = string.format('/Users/uriah/Downloads/precise-transit-6548/source/po_scan/a/%.3d/sweep.xyz', scan_num)
	...
	function extract_planes( points, points_map, normals, point_inds, iter, params )
		...
		torch.range(1,n_reference_points):apply( function( reference_idx )
			...
			if current_plane then
			    ...
			    print( "connected_components time: ", timer:time()['real']-t_cc )
				-- Check if we have enough points or do not lie within our own connected component set 
				if inliers < params.min_points_for_seed or is_inlier==0 then 
					inliers = 0
				end
			end
		    ...
		    print( "total time: ", timer:time()['real']-t_total )
	...
	local im_name = string.format("%s/patches_%.3d.jpg",output_dir, scan_num)
	print("saving "..im_name)
	image.save(im_name, patches_rgb)

	local im_name = string.format("%s/cc_planes_%.3d.jpg",output_dir, scan_num)
	print("saving "..im_name)
	image.save(im_name, cc_regions_final_rgb)

	plane_fname = string.format("%s/planes_%.3d.t7",output_dir, scan_num)
