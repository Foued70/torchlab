...
function FrameBuffer:read_pick_pixels(start_pixels, end_pixels)
  ...
  size[1] = end_pixels[1]-start_pixels[1]
  size[2] = end_pixels[2]-start_pixels[2]
  ...
  gl.ReadPixels(start_pixels[1], self.height-start_pixels[2], size[1], size[2], gl.RGB_INTEGER, gl.UNSIGNED_INT, img_data_ptr)
  gl.check_errors()
  gl.ReadBuffer(gl.NONE)
  self:unbind()

  -- object_id, triangle_id = submesh_start + primitive_id
  torch.add(pick_pixels:select(3,2), pick_pixels:select(3,2), pick_pixels:select(3,3))
  return pick_pixels:sub(1,pick_pixels:size(1), 1, pick_pixels:size(2), 1, 2):contiguous()
end
