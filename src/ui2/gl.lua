...
gl = setmetatable({}, {
  __index = function(tbl, name)
    ...
    -- must be a constant
    if not val then val = gl_lib['GL_'..name] end
...
gl.uint    = ffi.typeof('GLuint[?]')
gl.int     = ffi.typeof('GLint[?]')
gl.sizei   = ffi.typeof('GLsizei[?]')
gl.boolean = ffi.typeof('GLboolean[?]')
gl.enum    = ffi.typeof('GLenum[?]')
gl.clampf  = ffi.typeof('GLclampf[?]')
...
function gl.GetShaderiv(shader_id, name)
  ...
  gl_lib.glGetShaderiv(shader_id, name, value)
  return value[0]
end

function gl.GetProgramiv(program_id, name)
  local value = gl.int(1)
  gl_lib.glGetProgramiv(program_id, name, value)
  return value[0]
end

function gl.GetInfoLog(id, get_iv_func, get_log_func)
  local log_size  = get_iv_func(id, gl.INFO_LOG_LENGTH)
  local log_size_ptr = gl.sizei(1)
  if log_size == nil or log_size <= 0 then return "" end

  local log = gl.char(log_size+1)
  get_log_func(id, log_size+1, log_size_ptr, log)
  return ffi.string(log)
end

function gl.GetShaderInfoLog(shader_id)
  return gl.GetInfoLog(shader_id, gl.GetShaderiv, gl_lib.glGetShaderInfoLog)
end

function gl.GetProgramInfoLog(program_id)
