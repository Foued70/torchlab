...
function self.viewPlanesFaceOn(planes,pc)
	...
	for i,p in ipairs(planes) do
		...
		uv_maps[i], uv_lists[i] = self.flattenView(pc:get_xyz_list(p.mask),p.eqn[{{1,3}}])
...
function self.listToMap(uvlist,_res)
	...
	res = _res or torch.xcorr2(uvlist,torch.Tensor{-1,1}:reshape(2,1)):norm(2,2):min()
	uv_min = uvlist:min(1)
	msize = torch.floor((uvlist:max(1)-uv_min)/res):reshape(2)+2
	map = torch.zeros(msize[2],msize[1]) --the image map
	map_line = torch.Tensor(map:storage()) --represents the map data in an Nx1 representation

	--set the points to 1 with the linear indices
	lin_ind = torch.mv(torch.floor(torch.add(uvlist,-uv_min:expandAs(uvlist)):div(res):add(0.5)),torch.Tensor{1,map:size(2)}):add(1):long()
	map_line[lin_ind] = 1
	return map
end
...
function self.flattenView(xyz_list,dir,_res)
	...
	uvlist = torch.mm(xyz_list,torch.cat(u_axis,v_axis,2))

	return self.listToMap(uvlist,_res), uvlist
end
