...
function TextureBuilder:__init(scan, opts)
   ...
   local defaults = {
      ...
      check_frustrum_ppm = 10, -- separation of points to test for View overlap
...
function TextureBuilder:range_to_texture_dimensions(xrange, yrange, ppm, debug)
   local widthpx  = math.max(2,math.floor(xrange[3] * ppm + 1))
   local heightpx = math.max(2,math.floor(yrange[3] * ppm + 1))
...
function TextureBuilder:xyz_plane(fid, ppm, debug)
   ppm = ppm or self.ppm
   local model       = self.model
   ...
   local widthpx, heightpx, linx, liny, dx, dy = 
      self:range_to_texture_dimensions(xrange, yrange, ppm, debug)
...
function TextureBuilder:xyz_wireframe(fid,ppm)
   ...
   ppm = ppm or self.ppm -- pixels per meter
...
function TextureBuilder:build(fid)
   model = self.model
   views = model.views
   check_xyz = self:xyz_plane(fid,self.check_frustrum_ppm)
   plane_xyz = self:xyz_plane(fid,self.ppm)
   for vi = 1,#views do
      log.tic()
      view = views[vi]
...
function TextureBuilder:buildAll()
   for fid = 1,self.model.n_faces do 
      self:build(fid)
   end
end
