...
function TransformationValidation.validate_simple(best_pts, best_transformations, img_src_points, img_dest_points, angle1, angle2)
  ...
  local best_pts_temp = torch.Tensor(best_pts:size())
   ...
   local center1, center2, combined_i = util_sweep.warpAndCombine(torch.eye(3), img_src_points, img_dest_points)
  ...
  local angle_diff_orig = math.rad(diff)
   ...
   for i=1,table.getn(best_transformations) do
      local center1, center2, combined_i = util_sweep.warpAndCombine(best_transformations[i], img_src_points, img_dest_points)
      local angle_between
...
function TransformationValidation.validate(best_pts, best_transformations, img_src_points, img_dest_points, parameters)
   ...
   for k=1,table.getn(best_transformations) do
      ...
      scores_metrics_temp.anglediff_tmp[i] = util_sweep.findAngleDifference(opencv.Mat.new(torch.gt(combined_tmp[i][1],0)), opencv.Mat.new(torch.gt(combined_tmp[i][2],0)))
