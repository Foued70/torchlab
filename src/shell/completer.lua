...
completer.final_char_setter = function(char) end
...
table.insert(completers.value, function(t, sep)
...
table.insert(completers.value, function(t, sep)
  ...
  if mt and type(mt.__index) == "table" then
    return completers.value[1](mt.__index, sep) -- use regular table completer on metatable
  end
end)
...
if lfs then
  table.insert(completers.string, function(str)
...
function completer.complete(word, line, startpos, endpos)
  ...
  local function add_globals()
    ...
    call_completors(completers.value, _G)
  end
  ...
  local function contextual_list(expr, sep, str)
    if str then
      completer.final_char_setter('"')
      return call_completors(completers.string, str)
    end
    completer.final_char_setter("")
    if expr and expr ~= "" then
      local v = loadstring("return " .. expr)
      if v then
        call_completors(completers.value, v(), sep)
      end
    end
    if #matches == 0 then
      add_globals()
