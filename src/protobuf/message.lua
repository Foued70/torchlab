...
local function _AttachFieldHelpers(message_meta, field_descriptor)
    ...
    local is_packed = (field_descriptor.options and field_descriptor.options.packed)
    ...
    local _internal_parse = function(self, buffer, pos, pend)
        ...
        while pos ~= pend do
            ...
            if field_decoder == nil then
                print(message_meta._member.name, tag_bytes:byte(), pos, new_pos)
...
--    message_meta._name = descriptor.full_name
    ...
    message_meta._member.name = descriptor.full_name
